...
@export var default_pitch=1.0
@export var default_volume=0.25
...
func _process(delta):
	$Sphere.rotate_y(delta*deg_to_rad(spinning_speed))
	$Sphere.position.y+=floating_speed*delta
	if $Sphere.position.y<0.75:
		floating_speed=-floating_speed
	elif $Sphere.position.y>1.0:
		floating_speed=-floating_speed
		
func _on_area_3d_body_entered(body):
	if body.is_in_group("player") and is_collected==false:
		is_collected=true
		Global.is_possum_collected=true
